...
#region Members
  ...
  /// </summary>
  private void RunScript(List<double> xpositions, List<double> ypositions, List<int> plant_states, List<int> plant_type, object iterations, ref object plant_radius, ref object plant_heights, ref object plant_output_state, ref object plant_color)
  {
  ...
  public class LittleBluestem : Plant
  {
    ...
    public override float GetGrowth()
    {
      float sun_scaler = this.GetSunScaler();
      if(sun_scaler < 0) return -1;
      if(RandomInstance.Next(0, 100) > 5) // 5% magic
      {
